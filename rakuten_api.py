import time
from datetime import datetime
import requests
from logger import log_error, log_debug

from config import (
    RAKUTEN_APP_ID, RAKUTEN_ACCESS_KEY, RAKUTEN_API_URL, REQUEST_HEADERS,
    REQUEST_TIMEOUT, MAX_RETRY, BREAKFAST_KEYWORDS
)


def fetch_vacant_plans(
    hotel_no: str,
    checkin_date: str,
    checkout_date: str,
    adults: int,
    rooms: int = 1,
) -> list[dict] | None:
    """
    楽天トラベル空室ホテル検索APIを呼び出し、プラン一覧を返す
    まず朝食付きで検索し、0件なら全プランで再検索する
    価格は泊数合計（total × 泊数）で返す
    失敗時は None を返す
    """
    # 泊数を計算
    fmt = "%Y/%m/%d" if "/" in checkin_date else "%Y-%m-%d"
    nights = (datetime.strptime(checkout_date, fmt) - datetime.strptime(checkin_date, fmt)).days
    if nights <= 0:
        nights = 1

    params = {
        "applicationId": RAKUTEN_APP_ID,
        "accessKey": RAKUTEN_ACCESS_KEY,
        "hotelNo": hotel_no,
        "checkinDate": checkin_date.replace("/", "-"),
        "checkoutDate": checkout_date.replace("/", "-"),
        "adultNum": adults,
        "roomNum": rooms,
        "hits": 30,
        "format": "json"
    }

    try:
        # まず朝食付きで検索
        breakfast_params = {**params, "squeezeCondition": "breakfast"}
        response = _call_api_with_retry(breakfast_params)
        if response is None:
            return None

        plans = _extract_plans(response, nights)

        # 朝食付きプランが0件なら全プランで再検索
        if len(plans) == 0:
            log_debug(f"朝食付きプランなし。全プランで再検索: {hotel_no}")
            response = _call_api_with_retry(params)
            if response is None:
                return None
            plans = _extract_plans(response, nights)

        return plans

    except Exception as e:
        log_error(f"fetch_vacant_plans exception: {hotel_no} - {str(e)}")
        return None


def _call_api_with_retry(params: dict) -> dict | None:
    """
    APIを呼び出しリトライロジック付き
    成功時は レスポンスJSON（dict）を返す
    失敗時は None を返す
    """
    for attempt in range(MAX_RETRY):
        try:
            response = requests.get(
                RAKUTEN_API_URL,
                params=params,
                headers=REQUEST_HEADERS,
                timeout=REQUEST_TIMEOUT
            )

            if response.status_code == 429:
                wait_time = 2 ** attempt
                log_debug(f"HTTP 429 Rate Limit - waiting {wait_time} seconds")
                time.sleep(wait_time)
                continue

            if 400 <= response.status_code < 500:
                log_error(f"HTTP {response.status_code}: {response.text[:100]}")
                return None

            if 500 <= response.status_code < 600:
                log_debug(f"HTTP {response.status_code} - retrying...")
                if attempt < MAX_RETRY - 1:
                    time.sleep(1)
                    continue
                return None

            if response.status_code == 200:
                return response.json()

        except requests.exceptions.Timeout:
            log_error(f"Request timeout - attempt {attempt + 1}/{MAX_RETRY}")
            if attempt < MAX_RETRY - 1:
                time.sleep(1)
                continue
            return None

        except Exception as e:
            log_error(f"API request error: {str(e)}")
            return None

    return None


def _extract_plans(api_response: dict, nights: int = 1) -> list[dict]:
    """
    APIレスポンスからプラン情報を抽出
    roomBasicInfo と dailyCharge は交互に並んでいるのでペアで処理する
    価格は total（1泊1室） × 泊数 で返す
    """
    try:
        if "hotels" not in api_response or not api_response["hotels"]:
            return []

        plans = []

        for hotel_data in api_response["hotels"]:
            if "hotel" not in hotel_data or not hotel_data["hotel"]:
                continue

            for hotel_info in hotel_data["hotel"]:
                if "roomInfo" not in hotel_info:
                    continue

                basic_info = None
                for item in hotel_info["roomInfo"]:
                    if "roomBasicInfo" in item:
                        basic_info = item["roomBasicInfo"]
                    elif "dailyCharge" in item and basic_info is not None:
                        daily_charge = item["dailyCharge"]
                        plan_name = basic_info.get("planName", "")

                        # total = 1泊1室の合計金額（全人数分）
                        price_per_night = daily_charge.get("total")
                        if price_per_night is None:
                            price_per_night = daily_charge.get("rakutenCharge", 0) * basic_info.get("adultNum", 1)
                        if not price_per_night:
                            basic_info = None
                            continue

                        total_price = int(price_per_night) * nights

                        with_breakfast = (
                            basic_info.get("withBreakfastFlag", 0) == 1
                            or basic_info.get("breakfastSelectFlag", 0) == 1
                            or _check_breakfast(plan_name)
                        )

                        plans.append({
                            "price": total_price,
                            "plan_name": plan_name,
                            "has_breakfast": with_breakfast
                        })
                        basic_info = None

        plans.sort(key=lambda x: x["price"])
        return plans

    except Exception as e:
        log_error(f"Error parsing API response: {str(e)}")
        return []


def _check_breakfast(plan_name: str) -> bool:
    """planNameに朝食判定キーワードが含まれているかチェック"""
    plan_lower = plan_name.lower()
    for keyword in BREAKFAST_KEYWORDS:
        if keyword.lower() in plan_lower:
            return True
    return False
